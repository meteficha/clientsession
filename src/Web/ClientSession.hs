...
--
-- FIXME: DOCS
data Key = Key { aesKey :: !A.AES
               , keyRaw :: !S.ByteString
               }

instance Eq Key where
    Key _ r1 == Key _ r2 = r1 == r2

instance Serialize Key where
    put = putByteString . keyRaw
    get = either error id . initKey <$> getBytes 32
...
-- there isn't exactly 32 bytes (256 bits for AES).
--
-- Note that the input string is assumed to be uniformly chosen
-- from the set of all 32-byte strings.  In other words, each
...
initKey bs | S.length bs /= 32 = Left $ "Web.ClientSession.initKey: length of " ++
                                         show (S.length bs) ++ " /= 32."
initKey bs = Right $ Key { aesKey = A.initKey bs
                         , keyRaw = bs
                         }
...
encrypt key (IV iv) x = B.encode final
  where
    (encrypted, auth) = A.encryptGCM (aesKey key) iv S.empty x
    final             = toBytes auth `S.append` iv `S.append` encrypted
...
decrypt key dataBS64 = do
    ...
    guard (S.length dataBS >= 32) -- 16 bytes of IV + 16 bytes of AES-GCM auth tag
    let (auth, rest)       = S.splitAt 16 dataBS
        (iv, encrypted)    = S.splitAt 16 rest
        (decrypted, auth') = A.decryptGCM (aesKey key) iv S.empty encrypted
    guard (auth' == AuthTag auth)
    return decrypted
